...
def generate_release_trace(
    ...
    num_cloudlets: int = C.NUM_CLOUDLETS,
    surge_window: int = 6,
    surge_boost: float = 8.0,
) -> list[list[Request]]:
    """Release scenario: foundation models are released at random times.

    A model is unavailable (zero demand) before its release slot. Each release
    triggers a transient demand surge that decays over *surge_window* slots,
    stressing the cold-start handling of the recent demand estimate. Half of the
    catalog is available from the start so the cluster is never empty.
    """
    num_models = len(models)
    base_weights = zipf_weights(num_models, C.ZIPF_ALPHA)

    release_slot = np.zeros(num_models, dtype=int)
    half = num_models // 2
    perm = rng.permutation(num_models)
    # the second half of the (shuffled) catalog is released at random times
    release_slot[perm[half:]] = rng.integers(1, num_slots, size=num_models - half)

    trace = []
    for t in range(num_slots):
        available = np.where(release_slot <= t)[0]
        if len(available) == 0:
            trace.append([])
            continue
        w = base_weights[available].astype(float)
        # transient surge for freshly released models
        age = t - release_slot[available]
        fresh = (age >= 0) & (age < surge_window)
        w[fresh] *= surge_boost * (1.0 - age[fresh] / surge_window)
        w = w / w.sum()

        n_requests = max(1, rng.poisson(mean_requests))
        chosen = rng.choice(available, size=n_requests, p=w)
        service_types = rng.integers(0, C.NUM_SERVICE_TYPES, size=n_requests)
        if user_assignments is not None and len(user_assignments) > 0:
            user_idx = rng.integers(0, len(user_assignments), size=n_requests)
            homes = user_assignments[user_idx]
        ...
        trace.append(
            [
                Request(home=int(h), service_type=int(q), model_id=int(m))
                for h, q, m in zip(homes, service_types, chosen)
            ]
        )
    return trace
...
def generate_mixed_trace(
    ...
    """Mixed scenario combining the daily and release patterns.

    Two demand dynamics are superimposed on a single trace:
      * a recurring intra-day tidal pattern that modulates the *total* request
        volume with a ``peak_valley`` ratio (stresses the daily estimate), and
      * random model releases that each trigger a transient cold-start surge
        decaying over ``surge_window`` slots (stresses the recent estimate).
    Half of the catalog is available from the start so the cluster is never
    empty, while the other half is released at random times.
    """
    num_models = len(models)
    base_weights = zipf_weights(num_models, C.ZIPF_ALPHA)
    ...
    for t in range(num_slots):
        ...
        # tidal modulation of the total request volume
        slot_in_day = t % C.SLOTS_PER_DAY
        rate = mean_requests * daily_rate_multiplier(
            slot_in_day, C.SLOTS_PER_DAY, peak_valley
        )
        n_requests = max(1, rng.poisson(rate))
